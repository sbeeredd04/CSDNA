...
def generate_pie_chart(request):
    ...
    ax1.pie(category_sizes, labels=category_labels, autopct=lambda p: f'{p:.1f}%\n({p*sum(category_sizes)/100 :.0f})', 
            colors=['#ca6abd','#55A8E6', '#ffcc5c', '#96ceb4', '#ffeead'], 
            shadow=True, startangle=90, explode=[0.1] + [0] * (len(category_labels) - 1), 
            textprops={'color':'white'})
    ax1.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    ax1.set_title('Category Distribution', color='white')

    # Histogram for category counts
    ax2.bar(category_labels, category_sizes, color=['#ca6abd','#55A8E6', '#ffcc5c', '#96ceb4', '#ffeead'])

    # Adding the count above the bars in histogram
    for i in range(len(category_sizes)):
        ax2.text(i, category_sizes[i] + max(category_sizes) * 0.01, str(category_sizes[i]), ha='center', color='white')

    ax2.set_xlabel('Categories', color='white')
    ax2.set_ylabel('Number of Images', color='white')
    ax2.set_title('Image Count per Category', color='white')
    ...
    # Send buffer in a HTTP response to the browser with the mime type image/png set
